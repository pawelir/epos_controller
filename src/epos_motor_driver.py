...
class EposMotorDriver:
    ...
    def __init__(self, nodeID) -> None:
        self.nodeID = nodeID
        self.keyHandle = epos_lib.VCS_OpenDevice(
            b"EPOS4", b"MAXON SERIAL V2", b"USB", b"USB3", byref(self.pErrorCode)
        )
    ...
    def enable(self) -> None:
        epos_lib.VCS_SetProtocolStackSettings(
    ...
    def set_velocity_mode(self) -> None:
        epos_lib.VCS_ActivateProfileVelocityMode(
            self.keyHandle, self.nodeID, byref(self.pErrorCode)
        )
        epos_lib.VCS_SetVelocityProfile(
            self.keyHandle,
            self.nodeID,
            self.acceleration,
            self.deceleration,
    ...
    def get_avarage_velocity(self) -> int:
        ...
        pErrorCode = c_uint()
        epos_lib.VCS_GetVelocityIsAveraged(
            self.keyHandle, self.nodeID, byref(pVelocityIsAveraged), byref(pErrorCode)
        )
    ...
    def get_avarage_current(self) -> int:
        ...
        pErrorCode = c_uint()
        epos_lib.VCS_GetCurrentIsAveraged(
            self.keyHandle, self.nodeID, byref(pCurrentIsAveraged), byref(pErrorCode)
        )
...
class EposSubMotorDriver(EposMotorDriver):
    def __init__(self, nodeID, keyHandle0) -> None:
        self.nodeID = nodeID
        self.keyHandle = epos_lib.VCS_OpenSubDevice(
            keyHandle0, b"EPOS4", b"CANopen", byref(self.pErrorCode)
        )
